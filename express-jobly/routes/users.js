...
const { ensureAdminLoggedIn, ensureAdminOrUserLoggedIn } = require('../middleware/auth');
...
router.post('/:username/jobs/:jobId', ensureAdminOrUserLoggedIn, async function (req, res, next) {
	try {
		await User.applyJob(req.params.username, req.params.jobId);
