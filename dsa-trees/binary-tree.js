...
class BinaryTree {
   ...
   * the length of the shortest path from the root to a leaf. */
	...
	minDepth () {
		...
		function mindDepthHelper (node) {
			if (node.left === null && node.right === null) return 1;
			if (node.left === null) return mindDepthHelper(node.right) + 1;
			if (node.right === null) return mindDepthHelper(node.left) + 1;
			return Math.min(mindDepthHelper(node.left), mindDepthHelper(node.right)) + 1;
		}

		return mindDepthHelper(this.root);
	}
   ...
   * the length of the longest path from the root to a leaf. */
	...
	maxDepth () {
		...
		function maxDepthHelper (node) {
			...
			return Math.max(maxDepthHelper(node.right), maxDepthHelper(node.left)) + 1;
		}
